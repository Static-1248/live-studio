...
<template>
  <div class="h-screen w-full flex flex-row">
    ...
    <div class="flex flex-1 flex-col">
      <!-- 顶部横幅（占位） -->
      <div class="banner-container custom-bg flex items-center overflow-x-hidden" style="height: 60px;">
        <div class="w-100 overflow-x-visible">
	      <span class="banner-text text-white text-3xl font-bold modern-font whitespace-pre" 
               :style="{ animationDuration: `${bannerWidth * 0.015}s` }">
            {{ bannerText.replace(/\n/g, "    ") }}
	      </span>
		</div>
      </div>
